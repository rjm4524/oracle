		...
		<script>
			$(function(){
				...
				$("#minsertBtn").click(function(){
					...
					var idPtn = /^[a-z]{1}[a-zA-Z0-9]{3,15}$/;//첫글자 대문자에 최소 3~15자리까지 영소대문자 숫자 모두가능
					//var pwPtn = /^(?=.*[a-zA-Z])(?=.*[0-9])(?=.*[!@#$%^&*]).{3,}$/; //영문자,숫자,특수문자 1개이상
					//var f_tellPtn = /^[0-9]{2,3}$/;//유선전화 국번 02,043,010 
					//var m_tellPtn = /^[0-9]{3,4}$/; 
					if(!idPtn.test(id)){
						alert("1개의 영소문자 특수문자가 포함되어야합니다.");
						return false;
					}else {
						alert("정상입력되었습니다.")
					}					
		...
		<section>
			<form name="agree" method="get" action="join03_success.html">
				...
				<div id="info_input_button">
					...
					<input type="submit" id="minsertBtn" value="가입하기" />
				</div>
