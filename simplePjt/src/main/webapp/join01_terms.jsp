	...
	<head>
		...
		$(function(){
			$("#memBtn").click(function(){
				//alert("agree 체크 갯수 :"+$(".agree:checked").length);
				//alert("radio 체크 갯수 :"+$("input[type=radio]:checked").length);
				//alert("radio  갯수 :"+$("input[type=radio].length").length);
				if($(".agree:checked").length<3) {
					alert("모두 체크하셔야 다음으로 넘어가실수 있습니다.")
					return false;
				}
				alert("다음으로 진행합니다")
				agreeFrm.submit();
