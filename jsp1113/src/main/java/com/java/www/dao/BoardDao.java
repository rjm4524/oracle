...
public class BoardDao {
	...
	//게시글 전체 보내기 - 1개를 보내려면 dto , 여러개 보내려면 ArrayList
	public ArrayList<BoardDto> selectAll () {
		
		try {
			conn = getConnection();
			query = "select * from (select rownum rnum, a.* from(select * from board order by bno desc)a) where rnum >= 1 and rnum <= 20";
